...
\begin{frame}{Nonparametric models}
  \begin{itemize}
    \item If we can sample from the prior of a nonparametric model using finite resources with probability 1, then we can apply probabilistic programming (\eg rejection sampling is feasible)
    \vspace{\baselineskip}
    ...
    \end{itemize}
    \vspace{\baselineskip}
    \item Active research to produce finite sampling algorithms for other nonparametric processes (\eg negative binomial process \TBD{cite Koa and Dan})
  \end{itemize}
\end{frame}
...
\begin{frame}{Example: Infinite mixture of Gaussians}
  ...
  \begin{block}{Avoiding infinity}
    \vspace{0.5\baselineskip}
    \begin{itemize}
      \item $\Theta$ is now infinitely complex, and thus we cannot sample it directly
      \vspace{0.5\baselineskip}
